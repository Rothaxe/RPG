...
Dogged_Warcry = Dogged_Warcry or class({})

function Dogged_Warcry:OnSpellStart()
...
function Dogged_Warcry:OnProjectileHit_ExtraData(target, location, extra_data)
	if not target then
		return nil
	end
	...
	local ability = self
	--获得Allez的当前等级技能伤害
	local associated_ability = caster:FindAbilityByName("Allez")
	local damage = associated_ability:GetSpecialValueFor("Allez_damage")
