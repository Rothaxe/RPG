...
function BloodCryKnockBack: OnSpellStart()
	...
	local blast_pfx = ParticleManager:CreateParticle("particles/econ/items/outworld_devourer/od_ti8/od_ti8_santies_eclipse_area_shockwave.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function BloodCryKnockBack: OnHit(target, damage)
	local caster = self:GetCaster()
	local radius = self:GetSpecialValueFor("radius")
	local ability = self
